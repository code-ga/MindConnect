...
function AdminRequests() {
	...
	const {
		...
		queryFn: async () => {
			const { data, error } = await api.api["user-request"].all.get();
			if (error)
				throw new Error(error.value?.message || "Failed to fetch requests");
			return data.data;
		},
	...
	const processMutation = useMutation({
		mutationFn: async ({
			...
			id: string;
			status: Static<databaseTypes.databaseTypes["userRequest"]["status"]>;
			reason?: string;
			note?: string;
		}) => {
			const { data, error } = await api.api["user-request"]
				.process({ id })
				.put({ status, reason, note });
			if (error)
				throw new Error(error.value?.message || "Failed to process request");
			return data;
		},
		onSuccess: () => {
	...
	const requestChatMutation = useMutation({
		mutationFn: async ({
			...
			].post({ requestId, showSupporter });
			if (error)
				throw new Error(error.value?.message || "Failed to create chat room");
			return data.data;
		},
		onSuccess: (data) => {
		...
		<div className="container py-10 space-y-6">
			<div>
				...
				<p className="text-muted-foreground">
					Review and process user role and feature requests.
				</p>
			</div>
						...
						<CardHeader className="pb-2">
							<div className="flex items-center justify-between">
								<div>
									...
									<CardDescription>
										From User ID: {request.profileId} • Created on{" "}
									...
									<Dialog>
										...
										<DialogContent>
											<DialogHeader>
												...
												<DialogDescription>
													Create a private chat room to discuss this request
													with the user.
												</DialogDescription>
											</DialogHeader>
											<div className="flex items-center space-x-2 py-4">
												...
												/>
												<Label htmlFor="show-supporter">
													Show my profile information to the user
												</Label>
											</div>
											<DialogFooter>
												<Button
